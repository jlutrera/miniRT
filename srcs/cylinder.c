...
void intersect_cylinder(t_ray ray, t_cylinder *cy, t_point *t)
{
	...
	double t_base1 = intersect_disk(ray, vec_sub(vec(cy->coordinate.x, cy->coordinate.y, cy->coordinate.z), vec_mul(CD, -cy->height / 2)), CD, radius);
	double t_base2 = intersect_disk(ray, vec_add(vec(cy->coordinate.x, cy->coordinate.y, cy->coordinate.z), vec_mul(CD, cy->height / 2)), CD, radius);
