...
double compute_shadows(t_scene scene, t_vec P, t_vec N, t_vec D)
{
	...
	get_closest((t_ray){(t_point3){P.x, P.y, P.z}, vec_unit(L)}, scene.obj, &closest_obj, &(t_point){50, INFINITY}.x);
	if (closest_obj)
	{
		//Diffuse light
		dot_v = vec_dot(N, L);
...
// 	get_closest((t_ray){(t_point3){P.x, P.y, P.z}, vec_unit(L)}, scene.obj, &closest_obj, &(t_point){INFINITY, INFINITY}.x);
// 	if (closest_obj)
// 	{
// 		//Diffuse light
// 		dot_v = vec_dot(N, L);
